...
import {AAITenantsModel} from "../../support/jsonBuilders/models/aaiTenants.model";
...
describe('Change management AKA VNF changes', function () {
  ...
  let jsonBuilderAAITenantsModel: JsonBuilder<AAITenantsModel> = new JsonBuilder<AAITenantsModel>();
  ...
  beforeEach(() => {
    ...
    cy.readFile('/cypress/support/jsonBuilders/mocks/jsons/aaiGetTenants.json').then((res) => {
      jsonBuilderAAITenantsModel.basicJson(
        res,
        Cypress.config('baseUrl') + "/business/customers/customer/**/service-subscriptions/service-subscription/**",
        200,
        0,
        "aai-tenants")
    });
