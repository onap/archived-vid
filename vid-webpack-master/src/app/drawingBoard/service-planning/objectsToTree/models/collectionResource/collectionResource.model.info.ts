...
export class CollectionResourceModelInfo implements ILevelNodeInfo{
  ...
  getModel = (collectionResourceModelId: string, instance: CollectionResourceInstance, serviceHierarchy): CollectionResourceModel => {
    const uniqueIdOrName = this._sharedTreeService.modelUniqueIdOrName(instance, collectionResourceModelId);
    return new CollectionResourceModel(this._sharedTreeService.modelByIdentifier(serviceHierarchy, this.name, uniqueIdOrName));
  };
