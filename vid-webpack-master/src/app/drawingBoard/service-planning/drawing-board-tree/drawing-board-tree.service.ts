...
export class  DrawingBoardTreeService {
  ...
  isVFModuleMissingData(node: ITreeNode, serviceModelId : string): boolean {
    if(node.data.type === 'VF' &&!_.isNil(this.store.getState().service.serviceInstance[serviceModelId].vnfs) &&  !_.isNil(this.store.getState().service.serviceInstance[serviceModelId].vnfs[node.parent.data.vnfStoreKey])){
