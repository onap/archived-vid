  ...
  getModelInformation(serviceId: string, modelName: string, vfModuleModeNode:ITreeNode) {
    this._aaiService.getServiceModelById(serviceId).subscribe((result: any) => {
      ...
      this.modelInformations = [
        ...
        new ModelInformationItem("Invariant UUID", "invariantUuid", [this._sharedTreeService.getGetModelInvariantIdEitherFromInstanceOrFromHierarchy(vfModuleModeNode, this.model)], Constants.ServicePopup.TOOLTIP_INVARIANT_UUID, true),
