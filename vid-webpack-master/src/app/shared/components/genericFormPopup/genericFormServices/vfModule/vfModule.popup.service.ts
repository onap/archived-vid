  ...
  getModelInformation(serviceId: string, modelName: string, vfModuleModeNode:ITreeNode) {
    this._aaiService.getServiceModelById(serviceId).subscribe((result: any) => {
      ...
      this.modelInformations = [
        ...
        new ModelInformationItem("UUID", "uuid", [this._sharedTreeService.getUuidEitherFromInstanceOrFromHierarchy(vfModuleModeNode.data, this.model)], Constants.ServicePopup.TOOLTIP_UUID, true),
        new ModelInformationItem("Invariant UUID", "invariantUuid", [this._sharedTreeService.getInvariantUuidEitherFromInstanceOrFromHierarchy(vfModuleModeNode, this.model)], Constants.ServicePopup.TOOLTIP_INVARIANT_UUID, true),
