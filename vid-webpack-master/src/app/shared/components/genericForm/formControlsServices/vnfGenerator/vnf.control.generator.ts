...
export class VnfControlGenerator {
  ...
  getMacroFormControls(serviceId: string, vnfStoreKey: string, vnfName: string, dynamicInputs?: any[]): FormControlModel[] {
    ...
    if (!_.isNil(vnfModel)) {
      ...
      result.push(this._basicControlGenerator.getLcpRegionControl(serviceId, vnfInstance, result));
      result.push(this._basicControlGenerator.getLegacyRegion(vnfInstance));
      result.push(this._basicControlGenerator.getTenantControl(serviceId, vnfInstance));
      result.push(this.getPlatformMultiselectControl(vnfInstance, result, flags['FLAG_2002_VNF_PLATFORM_MULTI_SELECT']));
      result.push(this._basicControlGenerator.getLineOfBusinessControl(vnfInstance));
    }
    return result;
  }
  ...
  getAlaCarteFormControls(serviceId: string, vnfStoreKey: string, vnfName: string, dynamicInputs?: any[]): FormControlModel[] {
    ...
    if (!_.isNil(vnfModel)) {
      ...
      result.push(this.getPlatformMultiselectControl(vnfInstance, result, flags['FLAG_2002_VNF_PLATFORM_MULTI_SELECT']));
      result.push(this._basicControlGenerator.getLineOfBusinessControl(vnfInstance));
      result.push(this._basicControlGenerator.getRollbackOnFailureControl(vnfInstance));
    }
    return result;
  }
