...
describe('VFModule Control Generator', () => {
  ...
  test('Check for Volume Group Name Field - Given Ecomp Gen Name boolean - expect name to be null / moduleName_vol', () => {
    // let reduxState = new MockAppStore();
    ...
    // spyOn(service.vfModuleModel, 'getState').and.returnValue(reduxState.getState());
    const serviceId: string = "6e59c5de-f052-46fa-aa7e-2fca9d674c44";
    const vnfStoreKey: string = 'VF_vGeraldine 0';
    // const vfModuleStoreKey: string = 'vf_vgeraldine0..VfVgeraldine..base_vflorence..module-0';
    const uuidData: Object = {
    ...
    service.vfModuleModel.name = uuidData["modelName"];
    ...
    controls = service.buildFormByGivenState( [],vfModuleModel ,serviceId, vnf,vnfStoreKey);
    requiredExist = controls.find(ctrl => ctrl.controlName === FormControlNames.VOLUME_GROUP_NAME);
