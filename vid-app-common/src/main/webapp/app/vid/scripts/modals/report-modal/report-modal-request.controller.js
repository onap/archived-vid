...
(function () {
    ...
    appDS2.controller("reportModalInstanceController", ["$uibModalInstance", "$scope", "ReportService", "errorMsg", "requestInfo", reportModalInstanceController]);

    function reportModalInstanceController($uibModalInstance, $scope, ReportService, errorMsg, requestInfo) {
        const vm = this;

        const init = function() {
            vm.timestamp =  ReportService.getReportTimeStamp();
            vm.downloadEnable = false;
            ReportService.getReportDataWithId(requestInfo.requestId, requestInfo.instanceId).then(
                response => {
