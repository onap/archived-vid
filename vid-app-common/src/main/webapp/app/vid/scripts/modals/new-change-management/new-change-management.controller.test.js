...
describe('Testing workFlows from SO', () => {
  ...
  test('Verify load workflows from SO will call getSOWorkflow and return only names of workflows', () => {
    ...
    $changeManagementService.getSOWorkflows = jestMock.fn(() => getSOWorkflowsPromiseStub);
    $controller.workflows = [];
    // when
    $controller.loadSOWorkFlows();
    // then
    return getSOWorkflowsPromiseStub
    .then(() => {
  ...
  test('Verify load workflows will call load from SO and join workflow lists', () => {
    ...
    $changeManagementService.getWorkflows = jestMock.fn(() => getWorkflowsStub);
    $changeManagementService.getSOWorkflows = jestMock.fn(() =>  getSOWorkflowsPromiseStub);
    // when
    $controller.loadWorkFlows();
    // then
    return getWorkflowsStub
    .then(getSOWorkflowsPromiseStub)
    .then(() => {
  ...
  test('Verify broken SO workflows wont stop UI to render', () => {
    // given
    let getWorkflowsStub = Promise.resolve({"data": {"workflows": ["workflow 0"]}});
