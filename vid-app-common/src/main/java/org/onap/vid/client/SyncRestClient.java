...
public class SyncRestClient implements SyncRestClientInterface {
    ...
    @Override
    public <T> HttpResponse<T> put(String url, Map<String, String> headers, Object body, Class<T> aClass) {
        return callWithRetry(url, url2 -> Unirest.put(url2).headers(headers).body(body).asObject(aClass));
    }

    @Override
    public <T> HttpResponse<T> delete(String url, Map<String, String> headers, Class<T> aClass) {
