...
public class SyncRestClient {
    ...
    public HttpResponse<JsonNode> post(@NonNull String url, @NonNull Map<String, String> headers, @NonNull Object body) {
        return callWithRetry(url, url2 -> Unirest.post(url2).headers(headers).body(body).asJson());
    }

    public <T> HttpResponse<T> post(@NonNull String url, @NonNull Map<String, String> headers, @NonNull Object body, @NonNull Class<T> aClass) {
        return callWithRetry(url, url2 -> Unirest.post(url2).headers(headers).body(body).asObject(aClass));
    }

    public HttpResponse<JsonNode> get(@NonNull String url, @NonNull Map<String, String> headers, @NonNull Map<String, String> routeParams) {
    ...
    public <T> HttpResponse<T> get(@NonNull String url, @NonNull Map<String, String> headers, @NonNull Map<String, String> routeParams, @NonNull Class<T> aClass) {
        return callWithRetry(url, url2 -> {
            val getRequest = Unirest.get(url2).headers(headers);
            routeParams.forEach(getRequest::routeParam);
    ...
    public <T> HttpResponse<T> put(@NonNull String url, @NonNull Map<String, String> headers, @NonNull Object body, @NonNull Class<T> aClass) {
    ...
    private <T> HttpResponse<T> callWithRetry(String url, CheckedFunction1<String, HttpResponse<T>> httpRequest) {
        try {
            ...
            if (e.getCause() instanceof SSLHandshakeException) {
                return httpRequest.apply(url.replaceFirst(HTTPS_SCHEME, HTTP_SCHEMA));
            }
            throw e;
        }
    }
    ...
    private HttpClient initHttpClient() {
        try {
            ...
            logger.warn(EELFLoggerDelegate.debugLogger, DATE_FORMAT.format(new Date()) + CANNOT_INITIALIZE_CUSTOM_HTTP_CLIENT, e);
    ...
    private SSLContext trustOwnCACerts(String trustStorePass, KeyStore trustStore) throws NoSuchAlgorithmException, KeyManagementException, KeyStoreException, UnrecoverableKeyException {
    ...
    private KeyStore loadTruststore(String trustStorePath, String trustStorePass) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException {
