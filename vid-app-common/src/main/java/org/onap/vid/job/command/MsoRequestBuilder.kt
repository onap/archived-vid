...
@Autowired constructor(private val asyncInstantiationBL: AsyncInstantiationBusinessLogic,
    ...
    private fun generateMacroServiceInstantiationRequestParams(payload: ServiceInstantiation, serviceInstanceName: String?, jobId: UUID?): List<UserParamTypes> {
        val userParamsList: MutableList<UserParamTypes> = mutableListOf()

        if (featureManager.isActive(Features.FLAG_DISABLE_HOMING)) {
            userParamsList.add(ServiceInstantiationRequestDetails.HomingSolution(DISABLED_HOMING_VALUE))
        }

        userParamsList.addAll(generateServiceInstantiationServicesList(payload, serviceInstanceName,createServiceInstantiationVnfList(jobId, payload)))
