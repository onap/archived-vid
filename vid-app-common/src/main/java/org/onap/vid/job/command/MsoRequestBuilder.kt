...
@Autowired constructor(private val asyncInstantiationBL: AsyncInstantiationBusinessLogic,
    ...
    companion object {
        ...
        private const val HOMING_SOLUTION_KEY = "Homing_Solution"
    ...
    fun generateMacroServiceInstantiationRequest(jobId: UUID?, payload: ServiceInstantiation, optimisticUniqueServiceInstanceName: String, userId: String): RequestDetailsWrapper<ServiceInstantiationRequestDetails> {
        ...
        val userParamsList: MutableList<UserParamTypes> = serviceInstantiationServiceList.toMutableList()
        if (featureManager.isActive(Features.FLAG_DISABLE_HOMING)) {
            userParamsList.add(ServiceInstantiationRequestDetails.UserParamNameAndValue(HOMING_SOLUTION_KEY, DISABLED_HOMING_VALUE))
        }


        val requestParameters = ServiceInstantiationRequestDetails.RequestParameters(payload.subscriptionServiceType, false, userParamsList)
    ...
    private fun generateUserParamsNameAndValue(instanceParams: List<Map<String, String>>): List<ServiceInstantiationRequestDetails.UserParamNameAndValue> {
        ...
        val userParams = instanceParams.getOrElse(0, { emptyMap() }).map { x -> ServiceInstantiationRequestDetails.UserParamNameAndValue(x.key, x.value) }.toMutableList()
        if (featureManager.isActive(Features.FLAG_DISABLE_HOMING)) {
            userParams.add(ServiceInstantiationRequestDetails.UserParamNameAndValue(HOMING_SOLUTION_KEY, DISABLED_HOMING_VALUE))
        }
        return userParams
    }
