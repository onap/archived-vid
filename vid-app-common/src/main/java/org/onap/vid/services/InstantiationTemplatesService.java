...
public class InstantiationTemplatesService {
    ...
    public Collection<Service> setOnEachServiceInCollectionIsTemplateExistsValue(Collection<Service> services){

        Set<String> serviceModelIdsFromDB  = asyncInstantiationRepository.getAllTemplatesServiceModelIds();

        return services.stream().map(it -> setInServiceisHasTemplateValue(it, serviceModelIdsFromDB)).collect(Collectors.toList());
    }

    protected Service setInServiceisHasTemplateValue(Service service, Set<String> serviceModelIdsFromDb) {
