...
public class HealthCheckController extends UnRestrictedBaseController {
    ...
    public HealthStatus gethealthCheckStatusforIDNS() {
        ...
        try {
            ...
            return new HealthStatus(StatusCode.SUCCESS.getValue(), "health check succeeded");
        } catch (Exception ex) {

            LOGGER.error(EELFLoggerDelegate.errorLogger, "Failed to perform health check", ex);
            return new HealthStatus(StatusCode.FAILURE.getValue(), "health check failed: " + ex.toString());
        }
    }
    ...
    public HealthStatus getHealthCheck(
        ...
        try {
            ...
            LOGGER.debug(EELFLoggerDelegate.debugLogger, "count:::" + count);
            return new HealthStatus(StatusCode.SUCCESS.getValue(), dateFormat.format(new Date()), "health check succeeded");
        } catch (Exception ex) {

            LOGGER.error(EELFLoggerDelegate.errorLogger, "Failed to perform health check", ex);
            return new HealthStatus(StatusCode.FAILURE.getValue(), dateFormat.format(new Date()), "health check failed: " + ex.toString());
        }
    }
