...
public class BasicReportGenerator implements ReportGenerator {
	...
	ImmutableMap<String, Object> getFullSubscriberList() {
		try {
			...
			return ImmutableMap.<String, Object>builder()
					.put("status", fullSubscriberList.getStatus())
					.put("body", fullSubscriberList.getBody())
					.put("headers", fullSubscriberList.getHeaders())
					.build();
		} catch (Exception e) {
			return ImmutableMap.of("exception", e.toString());
		}
	}
}
