...
public class BasicReportGenerator implements ReportGenerator {
	...
	public Map<String, Object> apply(HttpServletRequest request, ReportCreationParameters creationParameters) {
		return ImmutableMap.<String, Object>builder()
				...
				.put("aaiFullSubscriberList", getFullSubscriberList(request))
				.put("userID", getUserIDFromSystemProperties(request))
				.put("commitInfo", getCommitInfoFromGitProperties())
				.put("sdcServiceModelsInfo", getSdcServiceModelsInfo())
	...
	String getSdcServiceModelsInfo() {
		try {
			HttpResponse<SubscriberList> fullSubscriberList = aaiService.getFullSubscriberList();
			return "Status: " + fullSubscriberList.getStatus() +
					", RawBody: " + fullSubscriberList.getRawBody() +
					", Headers: " + fullSubscriberList.getHeaders();
		} catch (Exception e) {
			return e.toString();
		}
	}

	String getFullSubscriberList(HttpServletRequest request) {
		try {
			RoleValidator roleValidator = RoleValidator.by(roleProvider.getUserRoles(request));
			SubscriberFilteredResults fullSubscriberList = aaiService.getFullSubscriberList(roleValidator);
			return fullSubscriberList.getSubscriberList().toString();
		} catch (Exception e) {
			return e.toString();
		}
	}
