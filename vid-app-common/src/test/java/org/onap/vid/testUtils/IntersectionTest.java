...
public class IntersectionTest {
    ...
    public void shouldWorkWithLargeData() {
        ...
        for (int i = 0; i < BIG_SIZE; i++) {
            input.add(Lists.newArrayList(Integer.toString(i)));
        }
        // when
        List<String> output = Intersection.of(input);
        // then
    ...
    @Test
    public void shouldWorkWithLargeDataWithNonEmptyResult() {
        // given
        List<List<String>> input = new ArrayList<>();
        for (int i = 0; i < BIG_SIZE; i++) {
            input.add(Lists.newArrayList("element"));
        }
        // when
        List<String> output = Intersection.of(input);
        // then
        assertThat(output).containsExactly("element");
    }
}
