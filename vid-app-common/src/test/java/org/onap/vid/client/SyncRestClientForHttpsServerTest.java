...
public class SyncRestClientForHttpsServerTest {
    ...
    public void testJsonResponseFromGet() throws JsonProcessingException {
        ...
        // when
        HttpResponse<JsonNode> jsonNodeHttpResponse = syncRestClient.get(securedUrl, Collections.emptyMap(), Collections.emptyMap());
        // then
        verifyHttp(stubServer).once(Condition.method(Method.GET), Condition.url(securedUrl), Condition.not(Condition.url(notSecuredUrl)));
    ...
    public void testObjectResponseFromGet() throws JsonProcessingException {
        ...
        String notSecuredUrl = "http://0.0.0.0:" + stubServer.getPort() + "/test";
        // when
        HttpResponse<SyncRestClientModel.TestModel> testModelHttpResponse = syncRestClient.get(securedUrl, Collections.emptyMap(), Collections.emptyMap(), SyncRestClientModel.TestModel.class);
        // then
        verifyHttp(stubServer).once(Condition.method(Method.GET), Condition.url(securedUrl), Condition.not(Condition.url(notSecuredUrl)));
    ...
    private HttpClient createNaiveHttpClient() throws GeneralSecurityException {
        ...
        final SSLConnectionSocketFactory socketFactory = new SSLConnectionSocketFactory(context, SUPPORTED_PROTOCOLS, null, SSLConnectionSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
