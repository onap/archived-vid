...
public class SyncRestClientForHttpServerTest {
    ...
    public void testJsonResponseFromGet() throws JsonProcessingException {
        ...
        // when
        HttpResponse<JsonNode> jsonNodeHttpResponse = syncRestClient.get(url, Collections.emptyMap(), Collections.emptyMap());
        // then
    ...
    public void testObjectResponseFromGet() throws JsonProcessingException {
        ...
        String url = "http://0.0.0.0:" + stubServer.getPort() + "/test";
        // when
        HttpResponse<SyncRestClientModel.TestModel> testModelHttpResponse = syncRestClient.get(url, Collections.emptyMap(), Collections.emptyMap(), SyncRestClientModel.TestModel.class);
        // then
        verifyHttp(stubServer).once(Condition.method(Method.GET), Condition.url(url));
    ...
    public void test404JsonResponseFromPost() throws JsonProcessingException {
        ...
        String url = "http://0.0.0.0:" + stubServer.getPort() + "/test";
        // when
        HttpResponse<JsonNode> jsonNodeHttpResponse = syncRestClient.post(url, Collections.emptyMap(), NOT_EXISTING_OBJECT);
        // then
    ...
    public void testObjectResponseFromPost() throws JsonProcessingException {
        ...
        String url = "http://0.0.0.0:" + stubServer.getPort() + "/test";
        // when
        HttpResponse<SyncRestClientModel.TestModel> objectHttpResponse = syncRestClient.post(url, Collections.emptyMap(), testObject, SyncRestClientModel.TestModel.class);
        // then
        verifyHttp(stubServer).once(Condition.method(Method.POST), Condition.url(url));
    ...
    public void testObjectResponseFromPut() throws JsonProcessingException {
        ...
        String url = "http://0.0.0.0:" + stubServer.getPort() + "/test";
        // when
        HttpResponse<SyncRestClientModel.TestModel> modelHttpResponse = syncRestClient.put(url, Collections.emptyMap(), testObject, SyncRestClientModel.TestModel.class);
        // then
        verifyHttp(stubServer).once(Condition.method(Method.PUT), Condition.url(url));
    ...
    public void testRedirectToHttp() throws JsonProcessingException {
        ...
        // when
        HttpResponse<JsonNode> jsonNodeHttpResponse = syncRestClient.get(secured_url, Collections.emptyMap(), Collections.emptyMap());
        // then
        verifyHttp(stubServer).once(Condition.method(Method.GET), Condition.url(available_url), Condition.not(Condition.url(secured_url)));
        assertEquals(jsonNodeHttpResponse.getStatus(), 200);
    }
    ...
    private void stubPostCall() throws JsonProcessingException {
        whenHttp(stubServer)
                .match(Condition.post("/test"), Condition.withPostBodyContaining(objectMapper.writeValueAsString(testObject)))
                .then(ok(), jsonContent(), contentType("application/json"));
    }

    private void stubPutCall() throws JsonProcessingException {
        whenHttp(stubServer)
                .match(Condition.put("/test"), Condition.withPostBodyContaining(objectMapper.writeValueAsString(testObject)))
