...
public class LoggerFormatTest extends BaseApiTest {
    ...
    public void validateAudit2019LogsFormat() {
        String logLines = validateLogsFormat(LogName.audit2019, "audit-ELS-2019.11", 0.6);
    ...
    private void moreValidationsForAuditFormat(String logLines) {
        List<String> logLinesAsList = splitLogLines(logLines);
        logLinesAsList.forEach(line->{
            String[] records = line.split("\\|");
            assertThat(records.length, equalTo(26));
            assertThat("RequestID shall be uuid", records[2], matchesPattern(BasePreset.UUID_REGEX));
            assertThat("server name shall be empty", records[5], emptyOrNullString());

            //authenticated request shall logs with userId.
            if (StringUtils.contains(records[6], "aai") || StringUtils.contains(records[6], "mso")) {
