...
public class InstantiationTemplatesApiTest extends AsyncInstantiationBase {
    ...
    public void templateTopology_givenDeploy_getServiceInfoReturnsCypressE2EFile() {
        ...
        assertThat(fetchRecentTemplateInfo(request.at("/modelInfo/modelVersionId").asText()), allOf(
            jsonEquals(templateInfoFromFile()).when(IGNORING_VALUES), // Just types
            jsonEquals(templateInfoFromFile().put("jobId", jobId)).whenIgnoringPaths(
                // The fields that are moving
                "id", "templateId",
    ...
    private JsonNode fetchRequestSummary(String serviceModelId) {
        return fetchRecentTemplateInfo(serviceModelId)
            .at("/requestSummary");
    }
